...
directorio <- "C:/Users/ldoli/Documents/_SoilGuard Elche/Sensor data" ## put your internal working directory

#### STEP 2: run this once correclty definend the working directory ####
library(lubridate)
library(readxl)
carpe <- "/a procesar" 
setwd(directorio)
...
for(i in 1:length(sensores)){
  ...
  ### ejercicio a partir de uno. Objeto: "archivo" ####
...
#### STEP 3: Output generation. Run all this ####
setwd(directorio)
datox <- do.call(rbind, datos_sensores)
write.csv(datox, paste0("SOILGUARD_ALL_sensors_",today(),".csv"), row.names = FALSE)